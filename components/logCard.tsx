        ...
        <div className="w-full ">
            <div className="mb-5">
                <div className="mx-auto max-w-[900px]">
                    {data?.map((item) => (
                        <div className="flex">
                            <p className="mr-2 min-w-[100px] max-w-[100px] py-2.5 text-base font-semibold text-[#3b3f5c] dark:text-white-light">{moment(item?.created_on).format('DD-MM-YYYY')}</p>
                            ...
                            <div className="self-center p-2.5 ltr:ml-2.5 rtl:ml-2.5 rtl:ltr:mr-2.5">
                                <div className="flex items-center">
                                    <div className={`${(item?.details || item?.opportunity?.remark) ? 'w-1/2 pr-2' : 'w-full pr-0'}`}>
                                        <div className="mt-4 w-full rounded-lg border border-gray-200 bg-white p-4  shadow-lg">
                                            <h3 className="text-lg font-semibold text-blue-700">{item.log_type}</h3>
                                            <p className="text-sm">
                                            ...
                                            <p className="text-sm">
                                                <strong>Created By:</strong> {capitalizeFLetter(item?.created_by?.username)}
                                            </p>
                                            {item?.opportunity_status ? (
                                                <p className="text-sm">
                                                    <strong>Opportunity Status:</strong> {item?.opportunity_status}
                                                </p>
                                            ) : (
                                                <p className="text-sm">
                                                    <strong>Stage:</strong> {item?.log_stage?.stage}
                                                </p>
                                            )}
                                            {/* {item?.log_stage && (
                                                <p className="text-sm">
                                    ...
                                    {(item?.details || item?.opportunity?.remark )&& (
                                        <div className="w-1/2">
                                            <div className="mt-4 w-full rounded-lg border border-gray-200 bg-white p-4  shadow-lg">
                                                {(item?.details || item?.opportunity?.remark) && (
                                                    <Tippy content={item?.detail  || item?.opportunity?.remark } placement="top" className="rounded bg-black p-1 text-white">
                                                        <p className=" flex text-sm">
                                                            <strong>Details:</strong>
                                                            <ReadMore children={item?.details  || item?.opportunity?.remark } />
                                                        </p>
                                                    </Tippy>
                                                )}
