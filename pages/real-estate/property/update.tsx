...
const UpdatePropertyPage=()=> {
  ...
  const propertyDetail = async () => {
    try {
      const res: any = await Models.property.details(1);
      getAddress(1);
      getImage(1);
      getVideo(1);
      getVirtualTour(1);
  ...
  const onSubmit = async () => {
    try {
      ...
      // const formData = buildFormData(plotBody);

      // const res: any = await Models.property.create(formData);
      // console.log("✌️res --->", res);
      // const images: any = await createImage(res?.id);
  ...
  const getAddress = async (id) => {
    try {
      ...
      const res = await Models.address.details(body);
      return res;
    } catch (error) {
      console.log("✌️error --->", error);
    }
  ...
  const getImage = async (property) => {
    try {
      ...
      const res = await Models.image.details(body);
      return res;
    } catch (error) {
      console.log("✌️error --->", error);
    }
  ...
  const getVideo = async (property) => {
    try {
      ...
      const res = await Models.video.details(body);
      return res;
    } catch (error) {
      console.log("✌️error --->", error);
    }
  ...
  const getVirtualTour = async (property) => {
    try {
      const body = {
        property: property,

        tour_url: state.video,
      };
                ...
                <div className="panel rounded-lg">
                  ...
                  <div className={`${"mt-4 grid grid-cols-2 gap-4"}`}>
                    <CustomSelect
                      ...
                      onChange={(e) => {
                        setState({ property_type: e, contact: "" });
                      }}
                ...
                <div className="panel rounded-lg p-6">
                  ...
                  <div className="mt-4 grid grid-cols-1 gap-4 md:grid-cols-2">
                    <TextInput
                      name="virtual_tour"
                      title="Virual Tour"
                      placeholder="Enter virual tour url"
...
export default PrivateRouter(UpdatePropertyPage)