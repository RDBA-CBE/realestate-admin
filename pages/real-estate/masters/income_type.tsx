...
const IncomeType = () => {
  ...
  const incomeTypeList = async (page) => {
    try {
      const body: any = {};
      if (state.search) body.search = state.search;
      const res: any = await Models.employmentType.list(page, body);
  ...
  const createIncomeType = async () => {
    try {
      ...
      await Models.employmentType.create(body);
      clearData();
      setState({ btnLoading: false });
      incomeTypeList(1);
      Success("Income type created successfully");
    } catch (error: any) {
  ...
  const updateIncomeType = async () => {
    try {
      ...
      await Models.employmentType.update(body, state.editId);
      clearData();
      setState({ btnLoading: false });
      incomeTypeList(state.page);
      Success("Income type updated successfully");
    } catch (error: any) {
      Failure(error?.name?.[0] || "Something went wrong");
      setState({ btnLoading: false });
    }
  ...
  const deleteRecord = async (row) => {
    try {
      setState({ btnLoading: true });
      await Models.employmentType.delete(row?.id);
      clearData();
      setState({ btnLoading: false });
      incomeTypeList(state.page);
      Success("Income type deleted successfully");
    } catch (error) {
      setState({ btnLoading: false });
    }
  };
  ...
  const handleDelete = (row) => {
    showDeleteAlert(
      ...
      "Are you sure want to delete this income type?",
    );
  };
  ...
  const handleNextPage = () => {
    if (state.next) incomeTypeList(state.page + 1);
  };

  const handlePreviousPage = () => {
    if (state.previous) incomeTypeList(state.page - 1);
  };
  ...
  return (
    <>
      ...
      <Modal
        addHeader={state.editId ? "Update Income Type" : "Create Income Type"}
        open={state.isOpen}
        close={() => clearData()}
        renderComponent={() => (
          <div className="pb-7">
            <form className="flex flex-col gap-3">
              ...
              <div className="mt-8 flex items-center justify-end">
                ...
                <button
                  type="button"
                  onClick={() => state.editId ? updateIncomeType() : createIncomeType()}
...
export default PrivateRouter(IncomeType);
