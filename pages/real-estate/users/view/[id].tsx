...
const UserView = () => {
    ...
    try {
      ...
      const allProperties = (res?.results || []).flatMap((list: any) =>
        (list.properties || []).map((p: any) => ({
          ...
          property_type:
            p.property_type?.map((t: any) => t.name).join(", ") || "-",
  ...
  const wishlistColumns = [
    ...
    },
    {
      ...
      render: (row: any) => (
        <button
          className="rounded-lg border border-[#9b0f09] px-3 py-1 text-xs font-semibold text-[#9b0f09] transition hover:bg-[#9b0f09] hover:text-white"
        ...
        <div className="space-y-4 lg:col-span-1">
          ...
          <div className="panel overflow-hidden rounded-2xl p-0 shadow-sm">
            ...
            <div className="flex flex-col items-center px-5 pb-5">
              <div className="mb-3 mt-3 flex h-16 w-16 items-center justify-center rounded-2xl border-4 border-white bg-[#7d0c07] text-3xl font-bold text-white shadow-lg">
              ...
              <div className="mt-3 flex items-center gap-2">
                <span
                  className={`flex items-center gap-1 rounded-full px-3 py-1 text-xs font-semibold ${statusColor.bg} ${statusColor.text}`}
                >
                  {statusColor.icon}
            ...
            <div className="grid grid-cols-2 divide-x border-t">
              <div className="flex flex-col items-center py-4">
                <p className="text-2xl font-bold text-[#9b0f09]">
          ...
          <div className="panel rounded-2xl shadow-sm">
            <h6 className="mb-4 text-sm font-bold uppercase tracking-wide text-gray-400">
              Contact Info
            </h6>
            <div className="space-y-3">
              {[
                ...
                // Location row only if location or address exists
                ...(user?.preferred_locations?.length>0 || user?.address
                  ? [
                      {
                        icon: <MapPin className="h-4 w-4" />,
                        label: "Preferred Location",
                        value: user?.preferred_locations?.length>0 && user?.preferred_locations?.map((item)=>item?.name)?.join(", ") || user?.address,
                      },
                    ]
                  : []),
                ...
                },
                {
                  ...
                  label: "Joined",
                  value: commonDateFormat(user?.created_at),
                },
              ]
          ...
          <div className="panel rounded-2xl shadow-sm">
            ...
            <div className="space-y-2">
              {[
                {
                  label: "User Type",
                  value: capitalizeFLetter(user?.user_type),
                },
                {
          ...
          <div className="panel rounded-2xl shadow-sm">
            ...
            <div className="mb-5 flex border-b border-gray-100">
              ...
              >
                ...
                Enquiries
                {state.enquiries?.length > 0 && (
                  <span className="rounded-full bg-[#9b0f09] px-2 text-[10px] font-bold text-white">
